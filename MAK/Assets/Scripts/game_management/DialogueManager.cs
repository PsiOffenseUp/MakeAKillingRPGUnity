...
public abstract class TextEffect
{
	...
	protected TextEffect(int start, int end) { this.span = new TimeSpan(start, end); }
}

public class WaveEffect : TextEffect
{
	public float waviness { get; private set; }
	public WaveEffect(int start, int end, float waviness) : base(start, end)
    {
...
{
	public SpinEffect(int start, int end) : base(start, end) { }
}

public class RainbowEffect : TextEffect
{
	public RainbowEffect(int start, int end) : base(start, end) { }

	//ROYGBIV
	public static readonly Color32[] rainbowColors = new Color32[]{ new Color32(255, 26, 5, 255), new Color32(255, 125, 18, 255),
		new Color32(251, 255, 31, 255), new Color32(13, 201, 0, 255), new Color32(17, 33, 209, 255),
		new Color32(174, 13, 255, 255), new Color32(191, 0, 124, 255)};
}
...
public class WaitEffect : TextTrigger
{
	...
	public WaitEffect() : base() { this.waitTime = 1.0f; }
	public WaitEffect(float wait_time) : base() { this.waitTime = wait_time; }
}
...
public class ChangeSpeedTrigger : TextTrigger
{
	...
	public ChangeSpeedTrigger() : base() { this.newSpeed = 4.0f; }
	public ChangeSpeedTrigger(float new_speed) : base() { this.newSpeed = new_speed; }
}

public class ResetSpeedTrigger : TextTrigger
{
	public ResetSpeedTrigger() : base() { }
}
...
public class TextEffectsMetadata
{
	...
	public void SetNewEffects(TMP_TextInfo text_info)
    {
		...
		for(i = 0; i < text_info.linkCount; i++)
        {
			...
			else if(linkId.StartsWith("resspd")) //Reset speed
            {
			...
			else if (linkId.StartsWith("rainbow"))
            {
				RainbowEffect re = new RainbowEffect(startChar, endChar);
				castEffect = (int)TextEffectType.RAINBOW;
				for (j = startChar; j < endChar; j++)
					textEffects[castEffect, j] = re;
			}
        }
    }
...
public class DialogueManager : MonoBehaviour, IUIElement
{
	//----------------------Member variables--------------------
	[SerializeField] GameObject textBoxParent, nameUI, bodyUi;
	[SerializeField] AudioClip talkingSoundEffect;
	[SerializeField] TMP_Text bodyText, nameText;
	[SerializeField] Material nameMaterial, bodyMaterial;
	[SerializeField] Color[] nameColors;
    ...
    void Update()
    {
		if (isActive)
		{
			if (ControlManager.TalkPressed()) //If the talk button is pressed
			{
	...
	//Adds all of the dialogue to the conversation in order
	public void AddConversation(BoogalooGame.Conversation conversation)
    {
		//Go through all of the dialogue and add it
		foreach (BoogalooGame.Dialogue dialogue in conversation.dialogues)
	...
	void ApplyTextEffects()
    {
		...
		for (int i = 0; i < currentCharacter && i < textInfo.characterCount; i++)
        {
			...
			{
				startVertIndex = charInfo.vertexIndex;
				tempVector = new Vector3(5.8f*Mathf.Cos(timeMult *Time.time + 1.2f*mesh.vertices[startVertIndex].x),
			...
			//Rainbow effect
			tempEffect = tEM.GetEffect(TextEffectType.RAINBOW, i);
			if (tempEffect != null)
			{
				startVertIndex = charInfo.vertexIndex;
				Color32 charColor = RainbowEffect.rainbowColors[((int)(7.0f * Time.time) + i) % 7];

				vertexColors[startVertIndex] = charColor;
				vertexColors[startVertIndex + 1] = charColor;
				vertexColors[startVertIndex + 2] = charColor;
				vertexColors[startVertIndex + 3] = charColor;
			}
		}
	...
	public IEnumerator NameSwapAnimation()
	{
		yield return null;
	}
	#endregion
